...
#include <stdarg.h>
#include <stdio.h>
uchar Graphic_Font = 0;
uchar xdata Graphic_FrameBuffer[1024];
void Graphic_SetFont(uchar f){
    Graphic_Font = f;
}
...
void Graphic_DrawLine(char x_1,char y_1,char x_2,char y_2,uchar op){
    double k,i,step=0;;       
    if(x_1==x_2&&y_1!=y_2){
        if(y_1>y_2) Graphic_Swap_int16(&y_1,&y_2);
        for(i=y_1;i<=y_2;i++) Graphic_DrawPoint(x_1,i,op);  
    }
    if(x_1!=x_2&&y_1==y_2){
        if(x_1>x_2) Graphic_Swap_int16(&x_1,&x_2);
    ...
    if(x_1!=x_2&&y_1!=y_2){
        ...
        if(abs(k)<=1){
            ...
            if(y_1>y_2) Graphic_Swap_int16(&x_1,&x_2);
...
void Graphic_DrawCircleWired(char x,char y,uchar r,uchar op){
    ...
    for(i=x-r;i<=x+r;i++){
        for(j=y-r;j<y+r;j++){
            ...
            if(fabs(dis-tmp)<1) Graphic_DrawPoint(i,j,op);
        }
    }    
   return;    
}
...
void Graphic_ShowString(int sx,int sy,uchar buffer[],uchar op){
    uchar i,dx;
    if(Graphic_Font==1) dx=8;
    else if(Graphic_Font==0) dx=6;
    else return;
    for(i=0;buffer[i]!='\0';i++) Graphic_ShowChar(sx+i*dx,sy,buffer[i],op);
    return;
}



void Graphic_Printf(int sx,int sy,uchar op,const char *str,...){
    uint8_t buffer[64];
    va_list args;
    va_start(args,str);
    vsprintf(buffer,str,args);
    va_end(args);
    Graphic_ShowString(sx,sy,buffer,op);
}
...
void Graphic_ShowNum(char sx,char sy,ulong num,uchar lenth,uchar mode,uchar op){
    ...
    for(i=lenth;i>0;i--){
        tmp=buffer[i-1];
        if(mode==0){
             Graphic_ShowChar(sx+i*dx,sy,tmp+48,op);
             nowx+=dx;
        }else{
