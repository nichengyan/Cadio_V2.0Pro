...
static uint8_t ErrorPos = 0xff; //错误位置
...
signed char __strcmp(const uint8_t* str1,const uint8_t *str2){
...
/*deepseek写的清晰的版本，但代价是多占用500字节code空间*/
...
uint8_t Cal_Get_Token(Cal_String *s, Cal_Array *tk)
{
    ...
    if (isalpha(current_char)) {
        ...
        /* 根据函数名设置函数标志 */
        if (__strcmp((char*)func_name, "sin") == 0) {
            Token_Tmp.value = SIN_FLAG;
        } else if (__strcmp((char*)func_name, "cos") == 0) {
            Token_Tmp.value = COS_FLAG;
        } else if (__strcmp((char*)func_name, "tan") == 0) {
            Token_Tmp.value = TAN_FLAG;
        } else if (__strcmp((char*)func_name, "asin") == 0) {
            Token_Tmp.value = ASIN_FLAG;
        } else if (__strcmp((char*)func_name, "acos") == 0) {
            Token_Tmp.value = ACOS_FLAG;
        } else if (__strcmp((char*)func_name, "atan") == 0) {
            Token_Tmp.value = ATAN_FLAG;
        } else if (__strcmp((char*)func_name, "abs") == 0) {
            Token_Tmp.value = ABS_FLAG;
        } else if (__strcmp((char*)func_name, "sinh") == 0) {
            Token_Tmp.value = SINH_FLAG;
        } else if (__strcmp((char*)func_name, "cosh") == 0) {
            Token_Tmp.value = COSH_FLAG;
        } else if (__strcmp((char*)func_name, "tanh") == 0) {
            Token_Tmp.value = TANH_FLAG;
        } else if (__strcmp((char*)func_name, "ln") == 0) {
            Token_Tmp.value = LN_FLAG;
        } else if (__strcmp((char*)func_name, "lg") == 0) {
            Token_Tmp.value = LG_FLAG;
        } else if (__strcmp((char*)func_name, "log") == 0) {
            Token_Tmp.value = LG_FLAG;
        } else if (__strcmp((char*)func_name, "rand") == 0) {
            Token_Tmp.value = RAND_FLAG;
        } else if (__strcmp((char*)func_name, "random") == 0) {
            Token_Tmp.value = RANDOM_FLAG;
        } else if (__strcmp((char*)func_name, "fact") == 0) {
...
Legacy 第一版我写的，码风很乱不利于维护。
缺乏更新支持，如定位字符串错误的功能就不再跟进了
于2025/10/27 弃用
*/
...
uint8_t Cal_Get_Token(Cal_String *s, Cal_Array *tk)
{
	...
	if (s->Str[s->Str_Ptr] >= '0' && s->Str[s->Str_Ptr] <= '9') // 如果是数字
	{
		...
		Token_Tmp.type = TYPE_NUMBER;
		if (Degree_Flg)
			Token_Tmp.value = strtod(Num_Tmp, NULL) * Pi / 180; // 角度
		else
			Token_Tmp.value = strtod(Num_Tmp, NULL); // 数值
		tk->token[tk->Array_Ptr++] = Token_Tmp;
		return TYPE_NUMBER;
	}
...
{
	int pa = 0, pb = 0;
	if (a.type <= TYPE_PARAMETER || b.type <= TYPE_PARAMETER || ErrorCode)
...
uint8_t Cal_Mid2Post(Cal_Array *Tokens, Cal_Array *Post)
{
	...
	for (i = 0; i < Tokens->Array_Ptr; i++)
	{
		...
		else if (Tokens->token[i].type == TYPE_PARAMETER)
		{ // 遇到参数，直接把参数扔进逆波兰式，不作处理
	...
	for (i = 0; i < Post->Array_Ptr; i++)
	{
		...
		if (Post->token[i].type == TYPE_FUNCTION)
		{
			...
			else if (Post->token[i].value == RANDOM_FLAG)
			{
				...
				res.value = rand() % (((int)numa.value - (int)numb.value + 1) + (int)numb.value);
				Stack_Push(&Stk, res);
			}
...
// 函数名：Calculate_Expression_Fast_Work(float *para)
// 传入参数：para:参数（数组下标1~6分别对应uvwxyz）
// 功能：代入参数计算结果
// 返回：计算结果
float FastCal_Work(float *para, uint8_t FastCal_Id)
{
	if (FastCal_Id > FastCal_Cnt - 1)
		return 0;
	return Cal_Calculate_RPN(&FastCal_Post[FastCal_Id], para);
}
uint8_t Get_ErrorCode()
{
	return ErrorCode;
}
uint8_t Get_ErrorPos()
{
	return ErrorPos;
}
