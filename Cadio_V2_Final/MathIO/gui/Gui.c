...
{ // 画弧形,弧度制，逆时针弧度递增
    ...
    if (theta1 >= theta2)
        return;
    dtheta = fabs(theta2 - theta1) / (float)r * 0.15;
...
void Gui_DrawRectangleRound(Page *p, int sx, int sy, int sizex, int sizey, int r, uchar op)
{ // 画圆角矩形框
    ...
    Gui_DrawLine(p, sx + r, sy + sizey - 1, sx + sizex - r, sy + sizey - 1, op);
    Gui_DrawLine(p, sx + sizex - 1, sy + r, sx + sizex - 1, sy + sizey - r , op);
}
...
{
    uint i;
    uint i_limit = p->PSizex * p->PSizey / 8;
    for (i = 0; i < i_limit; ++i)
