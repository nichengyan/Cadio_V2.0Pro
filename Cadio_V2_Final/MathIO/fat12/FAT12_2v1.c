...
void FAT12_WriteSector(uint32_t addr,uint8_t *dat){/*扇区地址*/
    W25QXX_EraseSector(addr*4096);
    W25QXX_WritePage(addr*4096+0*256,(unsigned char*)(dat+0*256));
    W25QXX_WritePage(addr*4096+1*256,(unsigned char*)(dat+1*256)); 
    W25QXX_WritePage(addr*4096+2*256,(unsigned char*)(dat+2*256));
    W25QXX_WritePage(addr*4096+3*256,(unsigned char*)(dat+3*256)); 
    W25QXX_WritePage(addr*4096+4*256,(unsigned char*)(dat+4*256));
    W25QXX_WritePage(addr*4096+5*256,(unsigned char*)(dat+5*256)); 
    W25QXX_WritePage(addr*4096+6*256,(unsigned char*)(dat+6*256));
    W25QXX_WritePage(addr*4096+7*256,(unsigned char*)(dat+7*256)); 
    W25QXX_WritePage(addr*4096+8*256,(unsigned char*)(dat+8*256));
    W25QXX_WritePage(addr*4096+9*256,(unsigned char*)(dat+9*256)); 
    W25QXX_WritePage(addr*4096+10*256,(unsigned char*)(dat+10*256));
    W25QXX_WritePage(addr*4096+11*256,(unsigned char*)(dat+11*256)); 
    W25QXX_WritePage(addr*4096+12*256,(unsigned char*)(dat+12*256));
    W25QXX_WritePage(addr*4096+13*256,(unsigned char*)(dat+13*256)); 
    W25QXX_WritePage(addr*4096+14*256,(unsigned char*)(dat+14*256));
    W25QXX_WritePage(addr*4096+15*256,(unsigned char*)(dat+15*256));  
}  
...
uint32_t FAT12_Modify(FAT12_File *f,uint8_t *dat,uint32_t num){//修改，即在文件内部写，不改变文件大小 
    uint16_t ClustNum,ByteNum,i;//整簇个数， 
	if(num>f->File_Size-f->File_CurrentOffset) num=f->File_Size-f->File_CurrentOffset;//超出部分丢弃
    FAT12_ReadSector(FAT12_NthClust(f,f->File_CurrentClusID)+5,FAT12_SharedBuffer);
    for(i=0;i<num;i++){
    	if(f->File_CurrentPos&&f->File_CurrentPos%FAT12_ClustSize==0){//恰好是整簇 
    	    FAT12_WriteSector(FAT12_NthClust(f,f->File_CurrentClusID)+5,FAT12_SharedBuffer);
    		f->File_CurrentClusID++;
    		f->File_CurrentPos=0;
    		FAT12_ReadSector(FAT12_NthClust(f,f->File_CurrentClusID)+5,FAT12_SharedBuffer);
    		//printf("1");
		}
		FAT12_SharedBuffer[f->File_CurrentPos]=(*(dat+i*sizeof(uint8_t)));
		f->File_CurrentPos++;
		f->File_CurrentOffset++;
	}
	FAT12_WriteSector(FAT12_NthClust(f,f->File_CurrentClusID)+5,FAT12_SharedBuffer);
