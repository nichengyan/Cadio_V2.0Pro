...
void FAT12_ReadSector(uint32_t addr,uint8_t*dat){/*扇区地址*/
    uchar i;
    for(i=0;i<16;i++) W25QXX_ReadPage(addr*4096+i*256,(unsigned char*)(dat+i*256)); 
}  
...
uint8_t FAT12_Fopen(FAT12_File *target,uint8_t FileName[]){
	...
	while(SecID<=6){
	    pstr=(FAT12_FDI*)(FAT12_SharedBuffer+Nth*32);
	...
	if(Flg){
		...
		while(1){
		    if(cnt>=FAT12_ClustBufferSize) break; 
			if(ClustNow==0x0fff) break;  
...
void ShowFileInfo(FAT12_File *p){
    uint8_t i=0,lb[20];;
    //OLED_ShowStr(0,0,p->File_Name,12);
    OLED_ShowNum(0,2,p->File_Size,5,16);
    OLED_ShowNum(64,2,p->File_FirstClus,5,16);
    OLED_ShowNum(0,4,FAT12_ClustBuffer[p->File_CurrentClusID],3,16);
    OLED_ShowNum(48,4,p->File_CurrentOffset,5,16);
    OLED_ShowNum(88,4,p->File_CurrentPos,5,16);
} 
//-----------------V2.0--------------------
//更新于2024/4/20
