...
void BM8563_ReadTime(time *tt){
    tt->sec=BM8563_ReadByte(0x02); tt->sec=((tt->sec>>4)&0x07)*10+(tt->sec&0x0f)*1;
    tt->min=BM8563_ReadByte(0x03); tt->min=((tt->min>>4)&0x07)*10+(tt->min&0x0f)*1;
    tt->hour=BM8563_ReadByte(0x04); 
    tt->mode=24;tt->hour=((tt->hour>>4)&0x0f)*10+(tt->hour&0x0f)*1;
    tt->day=BM8563_ReadByte(0x05); tt->day=((tt->day>>4)&0x03)*10+(tt->day&0x0f);
    tt->month=BM8563_ReadByte(0x07); tt->month=((tt->month>>4)&0x01)*10+(tt->month&0x0f);
    tt->weekday=BM8563_ReadByte(0x06); tt->weekday=tt->weekday&0x07;
    tt->year=BM8563_ReadByte(0x08); tt->year=tt->year;
}
