#include "Graphic.h"
static unsigned char xdata Graphic_FrameBuffer[1024];
...
void Grapgic_Printf(char sx,char sy,const char *str,...){
    char buffer[64];
    va_list args;
    va_start(args,str);
    ...
    va_end(args);
    sprintf("%s\n",buffer);
    Graphic_ShowString(0,2,buffer,16);
}
void Graphic_ShowNum(char sx,char sy,uint32_t num,uchar lenth,uchar mode,uchar op){
