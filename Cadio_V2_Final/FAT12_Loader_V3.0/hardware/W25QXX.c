...
uchar W25QXX_ReadRegisters(uchar id)
{
    ...
    CS = 0;
    if (id == 1)
        W25QXX_RWByte(0x05); // 读状态寄存器1
    if (id == 2)
        W25QXX_RWByte(0x35); // 读状态寄存器2
...
#if (SPIMODE == HardWare)
static uchar Flash_Select = 0; // Default 0;
...
void W25QXX_SelectChip(uchar FlashID)
{
    if (FlashID)
        Flash_Select = 1;
    else
