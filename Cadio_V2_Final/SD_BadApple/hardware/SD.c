...
uint8_t SPI_WriteReadByte(uint8_t dat) // MCU从SD卡读取一字节
{
    SPDAT = dat;
    while (!(SPSTAT & 0x80))
        ;          // 等待读取完成
    SPSTAT = 0xC0; // 令SPIF=1,WCOL=1清除标志
...
uint8_t SD_WaitReady(void)
{
    ...
    do
    {
        if (SPI_WriteReadByte(0XFF) == 0XFF)
            return 0; // OK
        t++;
    } while (t < 0XFF); // 等待
    return 1;
...
uint8_t SD_SendCmd(uint8_t cmd, uint32_t arg, uint8_t crc)
{
    ...
    if (cmd == CMD12)
        SPI_WriteReadByte(0xff); // Skip a stuff byte when stop reading
    // 等待响应，或超时退出
    Retry = 0X1F;
    do
    {
...
uint8_t SD_GetResponse(uint8_t Response)
{
    ...
    while ((SPI_WriteReadByte(0XFF) != Response) && Count)
        Count--; // 等待得到准确的回应
    if (Count == 0)
        return 1; // 得到回应失败
    else
 ...
 * 返回值：0,成功;其他,失败;
 * */
uint8_t SD_SendData(uint8_t *buf, uint8_t cmd)
{
    uint16_t t;
    if (SD_WaitReady())
        return 1; // 等待准备失效
    ...
    {
        for (t = 0; t < 512; t++)
            SPI_WriteReadByte(buf[t]); // 提高速度,减少函数传参时间
...
uint8_t SD_Init(void)
{
    ...
    SPI_Init(2); // 最快
    for (i = 0; i < 20; i++)
        SPI_WriteReadByte(0XFF); // 发送最少74个脉冲
    retry = 20;
    do
    {
    ...
    SD_Type = 0; // 默认无卡
    // OLED_Printf(0,2,16,"r1   %bd",r1);while(1);
    if (r1 == 0X01)
    {
        ...
        {
            for (i = 0; i < 4; i++)
                buf[i] = SPI_WriteReadByte(0XFF); // Get trailing return value of R7 resp
            if (buf[2] == 0X01 && buf[3] == 0XAA) // 卡是否支持2.7~3.6V
            {
                ...
                {
                    // OLED_Printf(0,0,16,"SD_SendCmd(CMD41,0x40000000,0X01) SUCCESS!\r\n");
                    for (i = 0; i < 4; i++)
                        buf[i] = SPI_WriteReadByte(0XFF); // 得到OCR值
                    /*
                    for(i = 0;i < 4;i++)
                      {
                        OLED_Printf(0,0,16,"buf[%d] %02X\r\n",i, buf[i]);
                      }
                      */
                    if (buf[0] & 0x40)
                        ...
                        SD_Type = SD_TYPE_V2;
                    // OLED_Printf(0,0,16,"SD_Type %02X\r\n", SD_Type);
                }
            }
        }
        else // SD V1.x/ MMC V3
...
uint8_t SD_WriteDisk(uint8_t *buf, uint32_t sector, uint8_t cnt)
{
    ...
    if (cnt == 1)
    {
        ...
        if (r1 == 0)                          // 指令发送成功
        {
            r1 = SD_SendData(buf, 0xFE); // 写512个字节
        }
    }
    else
    {
        ...
        if (r1 == 0)
        {
            do
            {
                r1 = SD_SendData(buf, 0xFC); // 接收512个字节
                buf += 512;
            } while (--cnt && r1 == 0);
            r1 = SD_SendData(0, 0xFD); // 接收512个字节
        }
    }
    SD_DisSelect(); // 取消片选
    return r1;      //
