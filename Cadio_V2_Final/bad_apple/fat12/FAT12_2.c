...
uint8_t FAT12_GetFreeCCCB()
{
    ...
    for (i = 0; i < FAT12_CCCB_Num; i++)
        if (FAT12_CCCB_Lenth[i] == 0)
            return i;
    return 0xff;
}
void FAT12_FreeCCCB(uint8_t i)
{
    if (i >= 0 && i < FAT12_CCCB_Num)
...
uint16_t FAT12_NthClust(FAT12_File *p, uint16_t n)
{
    ...
    {
        if (i == 0)
            pre[i] = FAT12_CCCB[p->File_CCCBId][0].Block_Size;
        else
            pre[i] = pre[i - 1] + FAT12_CCCB[p->File_CCCBId][i].Block_Size;
        if (pre[i] >= n)
            return FAT12_CCCB[p->File_CCCBId][i].Block_FirstClust + n - (pre[i] - FAT12_CCCB[p->File_CCCBId][i].Block_Size);
        ++i;
    }
    return 0xffff;
}
    ...
    while (SecID <= 6)
    {
        ...
        if (pstr->Attributes != 0x0F && pstr->Name[0] != 0xE5 && pstr->Name[0] != 0)
        {
            ...
            if (FAT12_MemoryCompare(NameTmp, FileName, 12) == 0xff)
            {
                ...
                if (FAT12_ActiveFileNum > FAT12_CCCB_Num || target->File_CCCBId == 0xff)
                    return 0xff;
                Flg = 1;
                break;
            }
